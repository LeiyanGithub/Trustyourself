...
def construct_prompt(args):
    ...
    source_file = ''
    target_path = f'{args.output_dir}/'
    if args.source == 'raw':
        target_file = f'{target_path}/{args.target}.json'
    ...
    if args.source == 'raw':
        ...
        for index in range(len(source_dataset)):
            for item in source_dataset[index]['pos']:
                source_dataset[index]['passage'] = item
            ...
            if 'neg' not in source_dataset[index]:
                continue
            for item in source_dataset[index]['neg']:
                source_dataset[index]['passage'] = item
                input_with_prompt = add_prompt(source_dataset[index], prompt)
                temp_prompts.append({
                    "idx": index,
